...
export default function ExperienceCard({ post }: ExperienceCardProps) {
  ...
  const getBadgeClass = (variant: string) => {
    ...
    // Nếu không có tweet nào
  if (tweets.length === 0) {
    return <div className="text-gray-500">No tweets available</div>;
  }
            ...
            {Array.from({ length: totalPages }).map((_, pageIndex) => (
              ...
              >
                {tweets
                  ...
                  .map((tweetId, idx) => (
                    <div key={idx} className="post-tweet min-h-[420px]">   {/* ← Thêm min-h này */}
        ...
        {totalPages > 1 && (
          <>
            <button
              type="button"
              onClick={prevSlide}
              className="absolute -left-3 top-1/2 -translate-y-1/2 z-20 bg-white dark:bg-zinc-900 border border-gray-300 dark:border-gray-600 rounded-full p-3 shadow-lg hover:bg-gray-50 dark:hover:bg-zinc-800"
            ...
            <button
              type="button"
              onClick={nextSlide}
              className="absolute -right-3 top-1/2 -translate-y-1/2 z-20 bg-white dark:bg-zinc-900 border border-gray-300 dark:border-gray-600 rounded-full p-3 shadow-lg hover:bg-gray-50 dark:hover:bg-zinc-800"
              disabled={totalPages <= 1}
              aria-label="Next"
            >
